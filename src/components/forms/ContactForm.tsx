        ...
        <>
            <form onSubmit={handleSubmit} className="flex flex-col items-center rounded justify-center p-8 bg-white text-gray-900 max-w-lg">
                <div className="w-full mb-2">
                    <h2 className="text-2xl mb-1">Please provide your basic contact information.</h2>
                </div>
                <div className="w-full my-2 flex flex-col items-start">
                    <label className="w-full flex flex-col my-1">
                        Name
                        <input name="name" type="text" placeholder="John Doe" onChange={handleChange} required className="border-b-2 border-gray-300 focus:border-green-500 outline-none p-2" />
                    </label>
                </div>
                <div className="w-full my-2 flex flex-col items-start">
                    <label className="w-full flex flex-col my-1">
                        Email
                        <input name="email" type="email" placeholder='example@web.com' onChange={handleChange} required className="border-b-2 border-gray-300 focus:border-green-500 outline-none p-2" />
                    </label>
                </div>
                <div className="w-full my-2 flex flex-col items-start align-center">
                    <label className="w-full flex text-sm gap-2">
                        Confirm email?
                        <input type='checkbox' name='toggle' value='toggle' required className="w-5 h-5 text-white accent-green-500"/>
                    </label>
                </div>
                ...
                <div className="self-start w-5/12 mt-4">
                    <button type="submit" className="w-full py-2 bg-green-500 text-white font-bold rounded">Submit</button>
                </div>
            </form>
